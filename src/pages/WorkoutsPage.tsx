...
export default function WorkoutsPage() {
	...
	const selectExercise = (name: string, muscleGroup: string) => {
			...
			<section className="workouts-page">
				<div className="workouts-header">
					<div>
						...
						<p>
							Log your training, track your strength, and build a history that
							actually matters.
						</p>
					</div>

					<button className="primary-action" onClick={startNewWorkout}>
						<Plus size={18} />
						Start Workout
					</button>
				</div>
				...
				<div className="workout-overview-grid">
					...
					<div className="workout-stat-card">
						...
						<strong>{workouts[0] ? workouts[0].name : "—"}</strong>
					</div>
				</div>
				...
				{workouts.length === 0 ? (
					<div className="empty-workout-card">
						...
						<p>
							Start your first workout and SanchFit will begin building your
							training history.
						</p>
					...
					<div className="workout-history-list">
						{(showHistory ? workouts : workouts.slice(0, 5)).map((workout) => (
		...
		<section className="workouts-page active-workout-page">
			<header className="active-workout-header">
				...
				<div className="active-workout-actions">
					<button className="secondary-action" onClick={cancelWorkout}>
						<X size={17} />
						Cancel
					</button>

					<button className="primary-action" onClick={finishWorkout}>
						<Check size={17} />
						Finish
					</button>
				</div>
			...
			<div className="exercise-list">
				{activeWorkout.exercises.length === 0 ? (
					<div className="empty-workout-card compact">
						...
						<p>Choose an exercise below to start logging your sets.</p>
					</div>
				) : (
							...
							<article className="exercise-card" key={exercise.id}>
								<div className="exercise-card-header">
									<button
										...
										onClick={() =>
											setExpandedExercise(expanded ? null : exercise.id)
										}
									>
									...
									<div className="exercise-content">
											...
											<div className="previous-performance">
												<div className="previous-performance-header">
													<div>
														...
														<strong>
															{previousPerformance.lastWorkout.date}
														</strong>
													</div>

													{previousPerformance.bestSet &&
														previousPerformance.bestWorkout && (
															<div className="previous-performance-best">
																<span>BEST</span>

																<strong>
																	{Math.round(
																		previousPerformance.bestSet.weight,
																	)}{" "}
																	{previousPerformance.bestWorkout.unit} ×{" "}
												...
												{previousPerformance.lastSets.length > 0 ? (
													<div className="previous-performance-sets">
														{previousPerformance.lastSets.map((set) => (
															<span
																key={set.id}
																className="previous-performance-set"
															>
																{set.weight}{" "}
																{previousPerformance.lastWorkout?.unit} ×{" "}
																{set.reps}
															</span>
														))}
													</div>
												) : (
													<span className="previous-performance-empty">
														No completed sets from the previous session.
													</span>
												)}
											</div>
										) : (
											<div className="previous-performance is-empty">
												<div>
													<span className="eyebrow">PREVIOUS PERFORMANCE</span>

													<strong>No previous data</strong>
												</div>

												<span>
													Complete this exercise once and SanchFit will remember
													it.
												</span>
											</div>
										)}
										...
										{exercise.sets.map((workoutSet, setIndex) => (
											...
											>
												...
												<select
													value={workoutSet.type}
													onChange={(event) =>
														updateSet(exercise.id, workoutSet.id, {
															type: event.target.value as SetType,
														})
													}
												>
													{Object.entries(setTypeLabels).map(
														([value, label]) => (
												...
												<div className="set-actions">
													<button
														className={`complete-set-button ${
															workoutSet.completed ? "is-complete" : ""
														}`}
														onClick={() =>
															updateSet(exercise.id, workoutSet.id, {
													...
													<button
														...
														onClick={() =>
															removeSet(exercise.id, workoutSet.id)
														}
												...
												{workoutSet.weight > 0 && workoutSet.reps > 0 && (
			...
			{showExercisePicker && (
				<div
					...
					onMouseDown={() => setShowExercisePicker(false)}
				>
					<div
					...
					>
						...
						<div className="custom-exercise">
							...
							<button className="secondary-action" onClick={addCustomExercise}>
								Create
							</button>
						</div>

						<div className="exercise-library">
							{exerciseLibrary.map(([name, muscleGroup]) => (
								<button
									className="exercise-library-item"
									key={name}
									onClick={() => selectExercise(name, muscleGroup)}
								>
									<span>
										<strong>{name}</strong>
